...
function drawCursor() {
  ...
  if (scrollbar.isInOuterBoundary(mouseX, mouseY) || scrollbar.selected)
    cursor("ew-resize");
  pop();
}
...
function calculatePower() {
  ...
  let k;
  if (lr.width * INCH_PER_UNIT < 2)
    k = (u - 0.8) * (lr.width * INCH_PER_UNIT - 0.36);
  if (lr.width * INCH_PER_UNIT >= 2)
    k = (u - 0.8) * (lr.width * INCH_PER_UNIT - 0.72);
  let holeArea = 0;
  shapes.forEach((shape) => {
  ...
  if (ppsi < 42)
    document.getElementById("powerpersqrinch").style = "color:black";
  else if (ppsi < 52)
    document.getElementById("powerpersqrinch").style = "color:orange";
...
document.getElementById("taskcircle").onclick = () => {
  addhole("circle");
};
document.getElementById("taskrect").onclick = () => {
  addhole("rect");
};
document.getElementById("taskvslot").onclick = () => {
  addhole("vslot");
};
document.getElementById("taskhslot").onclick = () => {
  addhole("hslot");
};
document.getElementById("taskterminal").onclick = () => {
  addhole("terminal");
};
document.getElementById("taskmirror").onclick = () => {
  itemKeyOperation("m");
};
document.getElementById("taskcenter").onclick = () => {
  itemKeyOperation("c");
};
document.getElementById("tasklock").onclick = () => {
  itemKeyOperation("l");
};
document.getElementById("tasksendmail").onclick = () => {
  holeExport();
  loadCustomItem();
  generateURL();
};
document.getElementById("lockall").onclick = () => {
  shapes.forEach((s) => (s.isLocked = true));
};
