class DomTable {
  ...
  build(num, isNew = true) {
    ...
    if (this.pos == null)
      this.pos = { top: this.ref.style.top, left: this.ref.style.left };
    else {
    ...
    document.getElementById("addgapbutton").onclick = () => {
      this.pos = { top: this.ref.style.top, left: this.ref.style.left };
      if (this.rows < 8) this.build(this.rows + 1, false);
    };
    document.getElementById("removegapbutton").onclick = () => {
      if (this.rows > 1) {
        this.pos = { top: this.ref.style.top, left: this.ref.style.left };
        if (this.rows > 1) this.build(this.rows - 1, false);
      }
    };
    document.getElementById("gapspan").addEventListener("click", function () {
      addCollapse(this);
    });
      ...
      while (r.gaps.length != num) {
        ...
        if (gl < num) {
          ...
          if (r.gaps.length == 2) {
            ...
            normalizeGap(ccg);
            this.buffer[0][0] = (
              (Math.abs(ccg.begin - ccg.end) * 360) /
              (2 * PI)
            ).toFixed(1);
            this.buffer[1][0] = Math.round(
              (((ccg.begin + ccg.end) / 2) * 360) / (2 * PI)
            );
            this.buffer[2][0] = ccg.t;
          }
          this.buffer[0].push(
            ((Math.abs(cg.begin - cg.end) * 360) / (2 * PI)).toFixed(1)
          );
          this.buffer[1].push(
            Math.round((((cg.begin + cg.end) / 2) * 360) / (2 * PI))
          );
  ...
  createTable(rows) {
    let firstPart =
      "<div id='gapboxdrag'class='dragpos' class = 'draggables'><div id='gapboxdragheader'class='headerthing'><span style='background-color:#F4A500; color:black;margin:2px;padding-left:2px;padding-right:2px;border-radius: 2px;'> 3</span> Gap / Sections / Fasteners <span id='gapspan'class = 'collapsible'>-</span></div><table class='ring-info' style='width:380px;display:block'><tr><td>#</td><td>Angle</td><td>Arc</td><td>Length</td><td>Closing</td></tr>";
    let lastPart =
      "<tr><td id='submitgapbutton'style='background-color:rgb(177, 0, 0);color:white'>Submit</td><td id='addgapbutton'>Add (+)</td><td id='removegapbutton'>Remove (-)</td></tr></table></div>";
    let middle =
      "<tr><td>" +
      1 +
      "</td><td><input class='input-data'id='gapangle" +
      0 +
      "' style='pointer-events:none;'> deg</td><td><input class='input-data'id='gapwidth" +
      0 +
      "'> deg</td><td><input class='input-data'id='gaplength" +
      0 +
      "'> in</td><td><select id='gaptype" +
      0 +
      "' name='type' style='width:50px;'><option value='barrel'>B-Nut</option><option value='qlatch'>Q-latch</option><option value='screws'>Tabs</option><option value='hinge'>Hinge</option><option value='partial'>Partial</option><option value='strap'>Strap</option></select></td></tr>";
    for (let i = 1; i < rows; i++) {
      middle +=
        ...
        "</td><td><input class='input-data'id='gapangle" +
        i +
        "'> deg</td><td><input class='input-data'id='gapwidth" +
        i +
        "'> deg</td><td><input class='input-data'id='gaplength" +
        i +
        "'> in</td><td><select id='gaptype" +
  ...
  loadTableData() {
    ...
    for (let i = 0; i < this.rows; i++) {
      ...
      if (!Number.isFinite(twidth) && !Number.isFinite(tangle)) {
        ...
        r.addGap(undefined, undefined, ttype, {
          position: (tangle * 2 * PI) / 360,
          angle: (twidth * 2 * PI) / 360,
        });
      } else {
        validentries = false;
        console.log("angle entries are invalid");
      }
    }
    if (validentries) {
  ...
  uploadTableData() {
    for (let i = 0; i < this.rows; i++) {
      document.getElementById("gapwidth" + i).value = (
        (Math.abs(r.gaps[i].begin - r.gaps[i].end) * 360) /
        (2 * PI)
      ).toFixed(1);
      this.setGapLength(i);
      document.getElementById("gapangle" + i).value = Math.round(
        (((r.gaps[i].begin + r.gaps[i].end) / 2) * 360) / (2 * PI)
      );
  ...
  create_Arc_Length_Dependency() {
    for (let i = 0; i < this.rows; i++) {
      document.getElementById("gapwidth" + i).onchange = (thing) =>
        this.setGapLength(thing.srcElement.id[thing.srcElement.id.length - 1]);
      document.getElementById("gaplength" + i).onchange = (thing) =>
        this.setGapWidth(thing.srcElement.id[thing.srcElement.id.length - 1]);
    }
  }
  setGapLength(index) {
    const angle =
      (document.getElementById("gapwidth" + index).value * 2 * PI) / 360;
    const radius = this.ringRadius;
    let length = Math.sqrt(
      2 * (radius * radius) - 2 * (radius * radius) * Math.cos(angle)
    );
    document.getElementById("gaplength" + index).value = (
      length * this.inchPerUnit
    ).toFixed(2);
  }
  setGapWidth(index) {
    ...
    let angle = Math.acos(
      (radius * radius * 2 - length * length) / (2 * radius * radius)
    );
    document.getElementById("gapwidth" + index).value = (
      (angle * 360) /
      (2 * PI)
    ).toFixed(1);
  }
...
class ComponentTableManager {
  constructor() {
    ...
    this.holeFields.forEach((h) => {
      h.onchange = () => this.changeHole();
    });
    ...
    this.terminalFields.forEach((h) => {
      h.onchange = () => this.changeConnector();
    });
    ...
    shapes.forEach((hole) => {
      if (hole.selected) {
        if (
          ...
          let rotation =
            (document.getElementById("c_rotation").value * 2 * PI) / 360;
