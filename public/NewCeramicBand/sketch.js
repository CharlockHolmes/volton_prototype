...
function saveImg() {
  localStorage.setItem(
    "image" + localStorage.getItem("cartIndex"),
    JSON.stringify(
      renderer.domElement
        .toDataURL("image/png")
...
function loadTerminal(
  ...
  loader.load(
    "ressources/" + name + ".glb", //'ressources/bornier.glb',
    (gltf) => {
      ...
      const pos = {
        x: Math.cos(angle) * radius * 1.151,
        y: Math.sin(angle) * radius * 1.151,
        z: offsetZ,
      };
...
function loadConnector(
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      if (name == "penture") {
        gltf.scene.position.x = Math.cos(angle) * radius * r.thickness * 1.15;
        gltf.scene.position.y = Math.sin(angle) * radius * r.thickness * 1.15;
        //gltf.scene.scale.y *=(r.radius*inchPerUnit*2+5)/10;
        gltf.scene.scale.x *= (r.radius * inchPerUnit * 2 + 5) / 10;
      } else {
        gltf.scene.position.x = Math.cos(angle) * radius * 1.1564;
        gltf.scene.position.y = Math.sin(angle) * radius * 1.1564;
      }
      gltf.scene.position.z = offsetZ;
      if (flipped && (name == "barrel" || name == "barrel_qlatch"))
        angle += CONNECTOR_ANGLE_OFFSET / r.radius;
      else if (name == "barrel" || name == "barrel_qlatch")
        angle -= CONNECTOR_ANGLE_OFFSET / r.radius;

      if (flipped && (name == "barrel_screw" || name == "barrel_screw_qlatch"))
        angle += CONNECTOR_ANGLE_OFFSET / r.radius;
      else if (name == "barrel_screw" || name == "barrel_screw_qlatch")
        angle -= CONNECTOR_ANGLE_OFFSET / r.radius;
  ...
  if (tr != null) {
    ...
    if (tc != null) {
      ...
      const position = {
        ...
        rotation: {
          _x: camera.rotation._x,
          _y: camera.rotation._y,
          _z: camera.rotation._z,
        },
        target: {
          x: controls.target.x,
          y: controls.target.y,
...
function defaultRing() {
  saveRing({
    ...
    resolution: 1000,
    gaps: [
      { begin: -0.0637, end: 0.0637, t: "barrel" },
      { begin: 3.07789, end: 3.2053, t: "barrel" },
    ],
    connectors: [
      {
        angle: -0.0637,
    ...
    terminals: [
      { angle: 0.5236, offset: 0.167, t: "bornier", rotation: 6.283 },
      { angle: 0.5236, offset: -0.167, t: "bornier", rotation: 1.571 },
      { angle: 5.75959, offset: 0.167, t: "bornier", rotation: 6.283 },
      { angle: 5.75959, offset: -0.167, t: "bornier", rotation: 1.571 },
    ],
    thickness: 0.9583333333333334,
    voltage: "120",
    power: "500",
  });
  //reload();
...
document.getElementById("practicering").onclick = () => {
  localStorage.setItem("tutorialStep", 14);
  blankRing();
};
function loadDefaultConnectorSettings() {
  const data = getStandardData(
    r.width * inchPerUnit,
    r.radius * 2 * inchPerUnit,
    "barrel"
  );
  connectors = [];
  r.connectors = [];
  ...
  r.gaps.forEach((gap) => {
    ...
    if (gap.t == "hinge")
      addConnector((gap.begin + gap.end) / 2, 0, "penture", false, randId0);
  });
}
...
function loadDefaultGapSettings(nb = 2, type = "barrel") {
  ...
  let gapOffset;
  if (nb == 1)
    gapOffset = arcToDeg_offset(r.radius * inchPerUnit, data.gap.single);
  if (nb >= 2)
    gapOffset = arcToDeg_offset(r.radius * inchPerUnit, data.gap.multiple);
  r.gaps = [];
...
function addRingClock() {
  ...
  const txtPosOffset =
    (rad - (1 / 10) * (r.radius * 2 >= 5 ? r.radius : 1)) / rad;
  addText(
    "0°",
    rad * Math.cos(0) * txtPosOffset,
    rad * Math.sin(0) * txtPosOffset
  );
  addText(
    "90°",
    rad * Math.cos(Math.PI / 2) * txtPosOffset,
    rad * Math.sin(Math.PI / 2) * txtPosOffset
  );
  addText(
    "180°",
    rad * Math.cos(Math.PI) * txtPosOffset,
    rad * Math.sin(Math.PI) * txtPosOffset
  );
  addText(
    "270°",
    rad * Math.cos((Math.PI * 3) / 2) * txtPosOffset,
    rad * Math.sin((Math.PI * 3) / 2) * txtPosOffset
  );

  addText(
    "Ø " + inchDiam.toFixed(2) + " in",
    (rad * Math.cos((Math.PI * 3) / 4) * txtPosOffset) / 2,
    (rad * Math.sin((Math.PI * 3) / 4) * txtPosOffset) / 2,
    (Math.PI * 7) / 4,
    { color: 0x003333 }
  );
  addText(
    "<< " + inchWide.toFixed(2) + " in >>",
    undefined,
    undefined,
    (Math.PI * 7) / 4,
    { color: 0x330033 },
  ...
  const clock0180Geo = new THREE.BoxGeometry(
    (rad * 2 * (rad - 1 / 9)) / rad,
    0.005,
    0.005
  );
  ...
  const clock90270Geo = new THREE.BoxGeometry(
    (rad * 2 * (rad - 1 / 9)) / rad,
    0.005,
    0.005
  );
  ...
  const clockDiameterGeo = new THREE.BoxGeometry(
    (rad * 2 * (rad - 1 / 12)) / rad,
    0.003,
    0.003
  );
  ...
  clockWidth.position.x =
    ((rad * (rad - 1 / 12)) / rad) * Math.cos((PI * 3) / 4);
  clockWidth.position.y =
    ((rad * (rad - 1 / 12)) / rad) * Math.sin((PI * 3) / 4);
...
function addText(
  ...
  textLoader.load("ressources/Roboto Black_Regular.json", function (font) {
    ...
    const centerOffset =
      -0.5 * (geoTxt.boundingBox.max.x - geoTxt.boundingBox.min.x);
...
function loadCustomItem() {
  ...
  //gui.hide();
  //Ring material
  const { positions, indices } = r.makeShape(segmentsAround, 1);
  ...
  cubes = [makeInstance(geometry, 0x81878c, 0)];
...
function render(time) {
  ...
  if (!plannerBoxFlag) {
    //Only renders if planner menu is hidden, fixes framerate issues
...
function reload() {
  if (enableLinkModification && window.location.search.length > 10)
...
function loadMenuThings() {
  document.getElementById("ringwidth").value = (r.width * inchPerUnit).toFixed(
    2
  );
  document.getElementById("ringdiameter").value = (
    r.radius *
    2 *
    inchPerUnit
  ).toFixed(2);
  ...
  setTimeout(() => {
    if (CAPTCHA_Validate()) {
      ...
      const data = {
        type: "ceramicband",
        url: compressed,
        power: document.getElementById("powerasked").value,
