...
let lock_img;
function preload() {
  barrel_img = loadImage("../NewMicaBand/ressources/2dtextures/barrel.png");
  barrel_f_img = loadImage("../NewMicaBand/ressources/2dtextures/barrel_f.png");
  barrel_screw_img = loadImage("../NewMicaBand/ressources/2dtextures/barrel_screw.png");
  barrel_screw_f_img = loadImage("../NewMicaBand/ressources/2dtextures/barrel_screw_f.png");
  barrel_qlatch_img = loadImage("../NewMicaBand/ressources/2dtextures/barrel_qlatch.png");
  barrel_qlatch_f_img = loadImage("../NewMicaBand/ressources/2dtextures/barrel_qlatch_f.png");
  terminal_img = loadImage("../NewMicaBand/ressources/2dtextures/terminal.png");
  hash_img = loadImage("../NewMicaBand/ressources/2dtextures/hash.png");
  scrollbar_img = loadImage("../NewMicaBand/ressources/2dtextures/scrollbar.png");
  rightarrow_img = loadImage("../NewMicaBand/ressources/2dtextures/rightarrow.png");
  leftarrow_img = loadImage("../NewMicaBand/ressources/2dtextures/leftarrow.png");
  lock_img = loadImage("../NewMicaBand/ressources/2dtextures/lock.png");
}
function setup() {
  pixelDensity(2);
...
function drawLastClick() {
  ...
  fill(255, 100, 100);
  if (lastClick.show) ellipse(lastClick.x, lastClick.y, 5);
  pop();
}
function drawBand() {
  push();
      ...
      if (shape.selected) {
        ...
        switch (key) {
          ...
          case "l":
            ...
            break;
          case "m":
            if (shape instanceof Connector) shapes.push(...shape.mirror());
...
function drawArrow(x, y, l, w, horizontal = true, txt = "") {
  ...
  textSize(TEXTSIZE);
  textAlign(CENTER, CENTER);
  if (typeof txt == "number") txt = txt.toFixed(3);
...
const yoff = [15, 15];
const _UPLEFT = {
  p1: { x: -xoff[0], y: -yoff[0] },
  p2: { x: -xoff[1], y: -yoff[1] },
  align: "right",
};
const _DOWNLEFT = {
  p1: { x: -xoff[0], y: yoff[0] },
...
function selectHole(hole) {
  if (
    ...
    if (hole.rotation != null)
      document.getElementById("c_rotation").value = Math.round(
        (hole.rotation * 360) / (2 * PI)
      );
...
function itemImport() {
  ...
  ter.forEach((t) => {
    ...
    shapes.push(
      new Terminal(pos.x, pos.y, t.flipped, t.t, 2 * PI - t.rotation)
    );
  });
  attachConnectors();
}
...
function holeExport() {
  ...
  shapes.forEach((shape) => {
    const yoff =
      Math.round((lrwidth / 2 - (shape.y / pheight) * lrwidth) * DECI) / DECI;
    if (shape.t === "circle") {
      holes.push({
      ...
      holes.push({
        r: {
          w: (shape.w / 2 / pheight) * lrwidth,
          h: (shape.h / 2 / pheight) * lrwidth,
        },
        offset: yoff,
        angle: shape.x * toRad,
        t: shape.t,
      ...
      holes.push({
        ...
        offset:
          Math.round(
            (lrwidth / 2 - ((shape.y + shape.h / 2) / pheight) * lrwidth) * DECI
          ) / DECI,
        angle: shape.x * toRad,
      ...
      shape.t == "barrel" ||
      shape.t == "barrel_screw" ||
      shape.t == "barrel_qlatch" ||
      shape.t == "barrel_screw_qlatch" ||
      shape.t == "penture"
    ) {
      /* if its a connector */
...
function holeImport() {
  ...
  holes.forEach((hole) => {
    ...
    if (hole.id == undefined) {
      if (hole.t == "circle") {
        shapes.push(
          new Circle(pos.x, pos.y, ((hole.r * 2) / lrwidth) * pheight)
        );
      }
      if (hole.t == "rect") {
        shapes.push(
  ...
  if (temp.length > 0) {
    ...
    restore.forEach((c) => {
      if (c.t == "circle") shapes.push(new Circle(c.x, c.y, c.d));
        ...
        shapes.push(new Vertical_Slot(c.x, c.y, c.w, c.h));
      else if (c.t == "hslot")
        shapes.push(new Horizontal_Slot(c.x, c.y, c.w, c.h));
      else if (
        c.t == "barrel" ||
        c.t == "barrel_screw" ||
        c.t == "barrel_qlatch" ||
        c.t == "barrel_screw_qlatch"
      )
        shapes.push(new Connector(c.x, c.y, c.flipped, c.t, undefined, c.id));
